...
public class TaxApproverService implements TaxRoleStartegy {
    private final TaxRepository taxRepository;

    @Override
    public boolean hasRole(String role) {
        return Role.APPROVER.name().equalsIgnoreCase(role);
    }

    @Override
    public Page<Tax> findAll(User user, Pageable pageable) {
        return taxRepository.findAllByStatus(TaxStatus.CHECKED, pageable);
    }
}
